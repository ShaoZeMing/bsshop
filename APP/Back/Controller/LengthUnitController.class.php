...
class LengthUnitController extends Controller
{
    ...
    public function lists()
    {
        ...
        //搜索处理
        $filter['filter_length_unit_name'] = I('get.filter_length_unit_name', '', 'trim');   //获取搜索关键词
        if ($filter['filter_length_unit_name'] !== '') {

            //需要搜索的字段length_unit_name
            $fd = explode(',', "length_unit_name");
            $cond['_logic'] = 'or';       //搜索条件之间为or（或）
            //遍历模糊搜索条件
            foreach ($fd as $f) {
                $cond[$f] = ['like', '%' . $filter['filter_length_unit_name'] . '%'];// 适当考虑索引问题
            }
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
        ...
        // 排序
        // 考虑用户所传递的排序方式和字段
        $order['field'] = I('get.field', 'length_unit_id', 'trim');// 初始排序, 字段
...
//        p($_REQUEST);
        ...
        foreach ($fd as $v){
            if ($operate=='check'.$v) {
                ...
                $title = I('request.'.$v, '');
                $cond[$v] = $title;
                // 判断是否传递了length_unit_id
                $length_unit_id = I('request.length_unit_id', null);
                if (!is_null($length_unit_id)) {
                    // 存在, 则匹配与当前ID不相同的记录
                    $cond['length_unit_id'] = ['neq', $length_unit_id];
                }
                // 获取模型后, 利用条件获取匹配的记录数
