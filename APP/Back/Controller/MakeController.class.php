...
//            $pk=$m_table->getPk();
            ...
            foreach ($field_info as $v){
                if($v['key']=='PRI'){$pk=$v['field'];} //获得主键
                $field_comment[]=empty($v['comment'])?$v['field']:$v['comment'];  //获得描述|字段名
                $fields[]=$v['field'];   //获得字段
            }

            //生成控制器
            //要替换的占位符
            $search=['___MODULE_','___TABLE_','___PK_','___TITLE_','___TABLEI_','___FIELD_','___FIELD_COMMENT_'];
            //替换的数据
            $replace=[$module,$model,$pk,$title,$table_i,$fields,$field_comment];
            //模板文件目录路劲
            ...
            //要替换的占位符
            $search=['___MODULE_','___TABLE_','___PK_','___TITLE_','___TABLEI_','___LIST_HEAD_','___LIST_BODY_'];
            //替换的数据
            $replace=[$module,$model,$pk,$title,$table_i,$head_html,$body_html];
