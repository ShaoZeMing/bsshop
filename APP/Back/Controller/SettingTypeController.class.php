...
class SettingTypeController extends Controller
{
    ...
    public function lists()
    {
        ...
        //搜索处理
        $filter['filter_setting_type_name'] = I('get.filter_setting_type_name', '', 'trim');   //获取搜索关键词
        if ($filter['filter_setting_type_name'] !== '') {

            //需要搜索的字段setting_type_name
            $fd = explode(',', "setting_type_name");
            $cond['_logic'] = 'or';       //搜索条件之间为or（或）
            //遍历模糊搜索条件
            foreach ($fd as $f) {
                $cond[$f] = ['like', '%' . $filter['filter_setting_type_name'] . '%'];// 适当考虑索引问题
            }
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
        ...
        // 排序
        // 考虑用户所传递的排序方式和字段
        $order['field'] = I('get.field', 'setting_type_id', 'trim');// 初始排序, 字段
...
//        p($_REQUEST);
        ...
        foreach ($fd as $v){
            if ($operate=='check'.$v) {
                ...
                $title = I('request.'.$v, '');
                $cond[$v] = $title;
                // 判断是否传递了setting_type_id
                $setting_type_id = I('request.setting_type_id', null);
                if (!is_null($setting_type_id)) {
                    // 存在, 则匹配与当前ID不相同的记录
                    $cond['setting_type_id'] = ['neq', $setting_type_id];
                }
                // 获取模型后, 利用条件获取匹配的记录数
