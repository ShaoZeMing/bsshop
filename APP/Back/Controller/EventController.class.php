...
class EventController extends Controller
{
    ...
    public function lists()
    {
        ...
        // $cond 表示用在模型中查询条件
        $cond = [];// 初始条件
        $filter['filter_event_name'] = I('get.filter_event_name', '', 'trim');
        if($filter['filter_event_name'] !== '') {
            $cond['event_name'] = ['like', '%'.$filter['filter_event_name'].'%'];// 适当考虑索引问题
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
    ...
    public function ajax()
    {
        ...
        switch ($operate) {
            ...
            case 'checkBrandUnique':
                // 获取填写的品牌名称
                $title = I('request.Event_name', '');
                $cond['Event_name'] = $title;
                // 判断是否传递了event_id
                $event_id = I('request.event_id', null);
                if (!is_null($event_id)) {
                    // 存在, 则匹配与当前ID不相同的记录
                    $cond['event_id'] = ['neq', $event_id];
                }
                // 获取模型后, 利用条件获取匹配的记录数
                $count = M('event_id')->where($cond)->count();
