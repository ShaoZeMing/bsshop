...
class MemberController extends Controller
{
    ...
    public function lists()
    {
        ...
        //搜索处理
        $filter['filter_member_name'] = I('get.filter_member_name', '', 'trim');   //获取搜索关键词
        if ($filter['filter_member_name'] !== '') {

            //需要搜索的字段member_email,member_telephone,member_password,member_name
            $fd = explode(',', "member_email,member_telephone,member_password,member_name");
            $cond['_logic'] = 'or';       //搜索条件之间为or（或）
            //遍历模糊搜索条件
            foreach ($fd as $f) {
                $cond[$f] = ['like', '%' . $filter['filter_member_name'] . '%'];// 适当考虑索引问题
            }
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
        ...
        // 排序
        // 考虑用户所传递的排序方式和字段
        $order['field'] = I('get.field', 'member_id', 'trim');// 初始排序, 字段
    ...
    public function edit()
    {
        ...
        if (IS_POST) {
            ...
            // 获取当前编辑的内容
            $member_id = I('get.member_id', '', 'trim');
            $this->assign('row', M('Member')->find($member_id));
    ...
    public function multi()
    {
        ...
        switch ($operate) {
            case 'delete':
                // 使用in条件, 删除全部的品牌
                $cond = ['member_id' => ['in', $selected]];
                M('Member')->where($cond)->delete();
                $this->redirect('lists', [], 0);
                break;
            default:
                # code...
...
//        p($_REQUEST);
        ...
        foreach ($fd as $v){
            if ($operate=='check'.$v) {
                ...
                $cond[$v] = $title;
                // 判断是否传递了member_id
                $member_id = I('request.member_id', null);
                if (!is_null($member_id)) {
                    // 存在, 则匹配与当前ID不相同的记录
                    $cond['member_id'] = ['neq', $member_id];
                }
                // 获取模型后, 利用条件获取匹配的记录数
