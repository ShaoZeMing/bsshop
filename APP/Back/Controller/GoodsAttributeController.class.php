...
class GoodsAttributeController extends Controller
{
    ...
    public function lists()
    {
        ...
        //搜索处理
        $filter['filter_goods_attribute_name'] = I('get.filter_goods_attribute_name', '', 'trim');   //获取搜索关键词
        if ($filter['filter_goods_attribute_name'] !== '') {

            //需要搜索的字段goods_attribute_name
            $fd = explode(',', "goods_attribute_name");
            $cond['_logic'] = 'or';       //搜索条件之间为or（或）
            //遍历模糊搜索条件
            foreach ($fd as $f) {
                $cond[$f] = ['like', '%' . $filter['filter_goods_attribute_name'] . '%'];// 适当考虑索引问题
            }
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
...
//        p($_REQUEST);
        ...
        foreach ($fd as $v){
            if ($operate=='check'.$v) {
                ...
                $title = I('request.'.$v, '');
                $cond[$v] = $title;
                // 判断是否传递了goods_attribute_id
                $goods_attribute_id = I('request.goods_attribute_id', null);
                if (!is_null($goods_attribute_id)) {
                    // 存在, 则匹配与当前ID不相同的记录
                    $cond['goods_attribute_id'] = ['neq', $goods_attribute_id];
                }
                // 获取模型后, 利用条件获取匹配的记录数
