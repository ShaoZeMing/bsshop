...
class SettingOptionController extends Controller
{
    ...
    public function lists()
    {
        ...
        //搜索处理
        $filter['filter_setting_option_name'] = I('get.filter_setting_option_name', '', 'trim');   //获取搜索关键词
        if ($filter['filter_setting_option_name'] !== '') {

            //需要搜索的字段setting_option__name
            $fd = explode(',', "setting_option__name");
            $cond['_logic'] = 'or';       //搜索条件之间为or（或）
            //遍历模糊搜索条件
            foreach ($fd as $f) {
                $cond[$f] = ['like', '%' . $filter['filter_setting_option_name'] . '%'];// 适当考虑索引问题
            }
        }
        // 分配筛选数据, 到模板, 为了展示搜索条件
        ...
        // 排序
        // 考虑用户所传递的排序方式和字段
        $order['field'] = I('get.field', 'setting_option_id', 'trim');// 初始排序, 字段
...
//        p($_REQUEST);
        ...
        foreach ($fd as $v){
            if ($operate=='check'.$v) {
                ...
                $setting_option_id = I('request.setting_option_id', null);
